...
- **Theme is vendored, not a gem.** This site uses Minima 3.0 theme files copied into `_layouts/`, `_includes/`, and `_sass/`. `theme: minima` is deliberately commented out in `_config.yml` so GitHub Pages / Actions builds use these local files with Jekyll 4.x (the legacy Pages builder only supports Jekyll 3.x). Do not re-enable the theme gem.
- **Editing styles:** put overrides in `_sass/minima/custom-styles.scss` and variable overrides in `_sass/minima/custom-variables.scss` (both currently placeholders). Avoid editing the vendored `_sass/minima/skins/*.scss` except that `auto.scss` was intentionally modernized to use `color.adjust()` instead of deprecated `lighten()`/`darken()` — preserve that when updating theme files.
- **Sass caveat:** the theme still uses deprecated `@import` (functional, warns). This is a known, tolerated state — see `_MaintenanceChecklist.md`. Don't churn it to `@use`/`@forward` without reason.
- **Pages** are the markdown files listed in `header_pages` (`index.md`, `bio.md`, `studying.md`, `events.md`), rendered via `_layouts/home.html` and `page.html`.
